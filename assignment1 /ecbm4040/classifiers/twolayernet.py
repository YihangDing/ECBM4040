...
class TwoLayerNet(object):
    ...
    def loss(self, X, y):
        ...
        #dense layer output def feedforward(self, X):
        dense_out=layer1.feedforward(X)
        #affine layer
        affine_out=layer2.feedforward(dense_out)
        affine_out-=np.max(affine_out)
        #softmax_loss(x, y):
        ...
        ###################################################
        #affinelayer  def backward(self, dout):
        affine_back=layer2.backward(dout)

        #denselayer   def backward(self, dout):
    ...
    def step(self, learning_rate=1e-5):
        ...
        #if optim == 'momentum':#momentum=0.9？
        #    momentum=0.9
        #    layer1.params[0]=layer1.params[0]= 
    ...
    def predict(self, X):
        ...
        #affine layer
        affine_out=layer2.feedforward(dense_out)
        #prediction=max index of affine ouput
        predictions=np.argmax(affine_out,axis=1)
