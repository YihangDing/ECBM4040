...
class RMSpropOptim(Optimizer):
    ...
    def step(self, model, learning_rate):
        ...
        for k in grads:
            ...
            #params=params-lr*grads/(cache+epsilon)^0.5
            params[k]-=learning_rate*grads[k]/np.power(cache[k]+eps,0.5)
...
class AdamOptim(Optimizer):
    ...
    def step(self, model, learning_rate):
        ...
        for k in grads:
            ...
            momentums_hat=momentums[k]/(1-np.power(beta1,t))
            velocitys_hat=velocitys[k]/(1-np.power(beta2,t))

            #update params
            params[k]-=learning_rate/(np.power(velocitys_hat,0.5)+eps)*momentums_hat
...
class NadamOptim(Optimizer):
    ...
    def step(self, model, learning_rate):
        ...
        for k in grads:
            ...
            #update params
            params[k]-=learning_rate/(np.power(velocitys_hat,0.5)+eps)*(beta1*momentums_hat+(1-beta1)*grads[k]/(1-pow(beta1,t)))
