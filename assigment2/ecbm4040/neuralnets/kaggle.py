...
def train_step(loss, learning_rate=1e-3):
    with tf.name_scope('train_step'):
        # step = tf.train.GradientDescentOptimizer(learning_rate).minimize(loss) tf.train.adamoptimizer
...
def my_LeNet(input_x, input_y, is_training,
    ...
    #is_training: calculate loss;
    #else: return output,None
...
def my_training_task5(X_train, y_train, X_val, y_val,
    ...
    iter_total = 0
    best_acc = 0
    cur_model_name = 'task5_{}'.format(int(time.time()))
...
def test(X_test, y_test,
    ...
    #output, _ = Net()  # we need to define the network architecture first, same as the training network
    ...
    print('number of batches for training: {}'.format(iters)) 
    # apply argmax on the output to generate the prediction, you should understand why we use 'argmax'
    #pred = predict(output) 
    eve = evaluate1(output, ys)
    prediction=[]
    merge_all=[]
    ...
    with tf.Session() as sess:
        ...
        for ite in range(iters):
            #test_batch_x = X_test[ite* batch_size : (1 + ite)* batch_size]
            #test_batch_y = y_test[ite* batch_size : (1 + ite)* batch_size]
